...
use pocketmine\network\mcpe\protocol\types\BlockPaletteEntry210;
...
class StartGamePacket210 extends DataPacket implements ClientboundPacket{
	...
	/** @var string */
	public $vanillaVersion = '1.16.210';
	/** @var int */
	...
	protected function decodePayload(PacketSerializer $in) : void{
		$this->entityUniqueId = $in->getActorUniqueId();
		$this->entityRuntimeId = $in->getActorRuntimeId();
		...
		[$this->spawnX, $this->spawnY, $this->spawnZ] = $in->getBlockPosition();
		...
		if ($this->session->getProtocolId() >= ProtocolInfo::PROTOCOL_1_16_210) $this->playerMovementSettings = PlayerMovementSettings::read($in);
		else $this->playerMovementSettings = new PlayerMovementSettings($in->getVarInt(), 0, false);
		...
		for($i = 0, $len = $in->getUnsignedVarInt(); $i < $len; ++$i){
			...
			$this->blockPalette[] = new BlockPaletteEntry210($blockName, $state);
		}
	...
	protected function encodePayload(PacketSerializer $out) : void{
		$out->putActorUniqueId($this->entityUniqueId);
		$out->putActorRuntimeId($this->entityRuntimeId);
		...
		if ($out->getProtocolId() >= ProtocolInfo::PROTOCOL_1_16_210) $this->playerMovementSettings->write($out);
		else {
			if ($this->playerMovementSettings instanceof PlayerMovementSettings) {
				$out->putVarInt($this->playerMovementSettings->getMovementType());
			} else $out->putVarInt(PlayerMovementType::LEGACY);
		}
		...
		foreach($this->blockPalette as $entry){
			...
			$out->put($nbtWriter->write($entry->getStates()));
		}
