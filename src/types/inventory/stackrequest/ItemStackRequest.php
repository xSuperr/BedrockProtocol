...
final class ItemStackRequest{
	...
	private static function readAction(PacketSerializer $in, int $typeId) : ItemStackRequestAction{
		if ($in->getProtocolId() <= ProtocolInfo::PROTOCOL_1_16_200 && $typeId >= 9) $typeId++; // MINE_BLOCK compensation
		switch($typeId){
	...
	public function write(PacketSerializer $out) : void{
		...
		foreach($this->actions as $action){
			...
			if ($out->getProtocolId() <= ProtocolInfo::PROTOCOL_1_16_200 && $typeId >= 10) $typeId--;
			$out->putByte($typeId);
			$action->write($out);
		}
