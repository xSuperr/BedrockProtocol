...
class PacketSerializer extends BinaryStream{
	...
	public function getSkin() : SkinData{
        ...
        if ($this->getProtocolId() >= ProtocolInfo::PROTOCOL_1_16_210) $skinPlayFabId = $this->getString();
        else $skinPlayFabId = '8a6bfa18-cfdd-46aa-a479-56b194cda178';
		...
		for($i = 0; $i < $animationCount; ++$i){
            ...
            if ($this->getProtocolId() >= ProtocolInfo::PROTOCOL_1_16_220) $expressionType = $this->getLInt();
            else $expressionType = 1;
	...
	public function putSkin(SkinData $skin) : void{
        ...
        if ($this->getProtocolId() >= ProtocolInfo::PROTOCOL_1_16_210) $this->putString($skin->getPlayFabId());
		...
		foreach($skin->getAnimations() as $animation){
            ...
            if ($this->getProtocolId() >= ProtocolInfo::PROTOCOL_1_16_220) $this->putLInt($animation->getExpressionType());
		}
	 ...
	 * @throws BinaryDataException
	 */
	public function getItemStackWithoutStackId() : Item|ItemStack{
    ...
    {
        [$netId, $netData] = ItemTranslator::getInstance()->toNetworkId($this->getProtocolId(), $item->getId(), $item->getMeta());
        ...
        if($netId === GlobalItemTypeDictionary::getInstance()->getDictionary($this->getProtocolId())->fromStringId("minecraft:shield")){
    ...
    public function getItem(): Item
    {
        ...
        [$id, $meta] = ItemTranslator::getInstance()->fromNetworkId($this->getProtocolId(), $id, $netData);
        ...
        if($nbtLen === 0xffff){
            ...
            if(!($decodedNBT instanceof CompoundTag)){
                throw new \UnexpectedValueException("Unexpected root tag type for item (1.16.210)");
            }
            $nbt = $decodedNBT;
        }elseif($nbtLen !== 0){
        ...
        if($id === GlobalItemTypeDictionary::getInstance()->getDictionary($this->getProtocolId())->fromStringId("minecraft:shield")){
            $this->getVarLong(); //"blocking tick" (ffs mojang)
        }
        if($nbt !== null){
            if($nbt->getInt('Damage', -1) !== -1){
                $meta = $nbt->getInt('Damage', 0);
                $nbt->removeTag('Damage');
