...
class AdventureSettingsPacket extends DataPacket implements ClientboundPacket, ServerboundPacket{
	...
	public const MINE_BUILD_200 = 0x01 | self::BITFLAG_SECOND_SET;
	...
	protected function decodePayload(PacketSerializer $in) : void{
		...
		if ($in->getProtocolId() <= ProtocolInfo::PROTOCOL_1_16_200) {
			if ($this->getFlag(self::MINE_BUILD_200)) {
				$this->setFlag(self::DEFAULT, true);
			}
		}
	...
	protected function encodePayload(PacketSerializer $out) : void{
		if ($out->getProtocolId() <= ProtocolInfo::PROTOCOL_1_16_200) {
			if ($this->getFlag(self::MINE) || $this->getFlag(self::BUILD) || $this->getFlag(self::DEFAULT)) {
				$this->flags = self::MINE_BUILD_200;
			}
		}
